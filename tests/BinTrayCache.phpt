...
class BinTrayCacheTest extends TestBase
{
    function testGetFileDetails() {
        $files = BinTrayCache::getFileDetails(
        ...
        foreach($files as $index => $file) {
            ...
            Assert::equal($index, $file->priority);
        }
    ...
    function testSortByVersionDate() {
        ...
        $files = BinTrayCache::getFileDetails($json);
        ...
        Assert::equal(1, $files[0]->priority);
        Assert::equal(2, $files[1]->priority);
        Assert::equal(0, $files[2]->priority);
    }
    ...
    function testNoMatches() {
        ...
        Assert::exception(function() use($json) {
            BinTrayCache::getFileDetails($json);
        }, 'RuntimeException', '#Unable to find file#');
    }

    function testEmptyDownload() {
        Assert::exception(function() {
            BinTrayCache::getFileDetails('[]');
        }, 'RuntimeException', '#Unable to find file#');
    }

    function testInvalidFiles() {
        Assert::exception(function() {
            BinTrayCache::getFileDetails('[{"name":"boost.tar.gz"},');
        }, 'RuntimeException', '#Error parsing#');
    }
}

$test = new BinTrayCacheTest();
$test->run();
